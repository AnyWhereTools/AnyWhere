---
name: anywhere-author
description: >
  Author AnyWhere Finder right-click actions by directly editing its local data
  files. Use when the user wants an AI to add / edit / remove a AnyWhere
  right-click action, or implement a Finder capability "in AnyWhere". AnyWhere
  stores everything as local files and live-reloads them within ~3s, so editing
  the files IS the API — no CLI, no restart.
---

# Authoring AnyWhere actions by editing its files

AnyWhere (a macOS Finder right-click manager) keeps **all state as plain local files** and
**watches `config.json`'s mtime every ~3s** — so when you edit these files, a running AnyWhere
picks up the change automatically (no restart, no import). This skill is how an AI implements a
AnyWhere capability for the user.

## Data location

```
~/Library/Application Support/AnyWhere/
...
| `ANYWHERE_VARIANT` | the chosen submenu value (when the action has `variants`) |
| `ANYWHERE_DATA` / `ANYWHERE_TEMPLATES` | the data / templates directories |
| `ANYWHERE_SCRIPT` | this script's absolute path → `pack_root="${0:A:h}"` to find sibling files/binaries |
...
Reference inputs via `"$@"` / `$ANYWHERE_PATHS`. **Never** build commands by string-interpolating
...
- **Submenu (fixed list)**: add `"variants": { "fixed": { "_0": ["png", "jpeg", "heic"] } }` — each value arrives as `$ANYWHERE_VARIANT`
...
- `placement` — `topLevel` or `submenu` (groups under a "AnyWhere ▸" submenu).
...
```bash
SUPP="$HOME/Library/Application Support/AnyWhere"
...
AnyWhere reloads within ~3s — the action appears in Finder's right-click menu. (If it doesn't,
the JSON was invalid; AnyWhere keeps the old config and shows an error — restore `$CFG.bak`,
...
ANYWHERE_PATHS="/some/file.txt" /bin/zsh "$SUPP/Scripts/word-count.sh" /some/file.txt  # dry-run the script
```
...
- Don't simultaneously edit actions in the AnyWhere UI while editing files (last writer wins).
