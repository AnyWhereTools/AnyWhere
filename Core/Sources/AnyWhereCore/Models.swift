...
public struct MatchRule: Codable, Equatable, Sendable {
    ...
    public init(targets: TargetKind = .any, utis: [String] = [],
        ...
        self.extensions = extensions; self.filenamePattern = filenamePattern
    ...
    /// User-entered suffixes are literal, never glob or regular expressions.
    ...
    public static func parseExtensions(_ text: String) -> [String] {
        var seen = Set<String>()
        return text.components(separatedBy: CharacterSet.whitespacesAndNewlines.union(CharacterSet(charactersIn: ",，;；")))
            .filter { !$0.isEmpty }.map(normalizedExtension).filter { seen.insert($0).inserted }
    }

    public static func isValidExtension(_ value: String) -> Bool {
        let suffix = normalizedExtension(value)
        let allowed = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: ".-_+"))
        return !suffix.isEmpty && suffix.unicodeScalars.allSatisfy(allowed.contains)
            && suffix.split(separator: ".", omittingEmptySubsequences: false).allSatisfy { !$0.isEmpty }
    }

    /// Explicitly editing suffixes replaces legacy type filters; unrelated edits preserve them.
    public mutating func setUserExtensions(_ values: [String]) {
        extensions = values.map(Self.normalizedExtension)
        utis = []
        filenamePattern = nil
    }
}
...
public extension UUID {
    ...
    static func deterministic(_ name: String) -> UUID {
        let digest = SHA256.hash(data: Data("com.anywhere.preset.\(name)".utf8))
...
public extension MenuConfig {
    ...
    static func defaultSeed() -> MenuConfig {
        ...
        // 扩展包规范和上游参考项目见 README 与 docs/pack-spec.md。
