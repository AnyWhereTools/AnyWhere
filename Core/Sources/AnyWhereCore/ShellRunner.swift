...
public enum ShellRunner {
    ...
    /// 脚本环境契约（spec §3.2.1）：
    /// - 外部脚本一律由 zsh 解释执行（无需可执行位），路径经 ANYWHERE_SCRIPT 引用，禁止拼接进命令文本
    /// - $1..$n 与 ANYWHERE_PATHS 传选中路径；ANYWHERE_VARIANT 传子菜单值
    /// - 调用方经 extraEnv 注入 ANYWHERE_TEMPLATES / ANYWHERE_DATA
    /// - 契约 env（ANYWHERE_VARIANT 等）覆盖 extraEnv，保证执行层语义不被调用方覆盖
    public static func runScript(_ spec: ScriptSpec, paths: [String], variant: String?,
        ...
        var env = extraEnv
        env["ANYWHERE_PATHS"] = paths.joined(separator: "\n")
        env["ANYWHERE_VARIANT"] = variant ?? ""
        let command: String
        if let resolved = spec.resolvedScriptPath(base: scriptBase) {
            env["ANYWHERE_SCRIPT"] = resolved
            command = #"/bin/zsh "$ANYWHERE_SCRIPT" "$@""#
        } else {
            command = spec.inlineSource ?? "true"
        }
        return run("/bin/zsh", ["-lc", command, "anywhere"] + paths,
