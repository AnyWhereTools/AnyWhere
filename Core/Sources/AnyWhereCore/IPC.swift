...
public enum IPC {
    public static let actionNotification = "com.anywhere.action"
    public static let heartbeatNotification = "com.anywhere.heartbeat"
    /// 主 App → 扩展：推送 ExtensionSnapshot 的分块（ChunkedTransport.Chunk JSON）
    public static let snapshotNotification = "com.anywhere.snapshot"
    /// 扩展 → 主 App：请求立即推送一份快照（扩展启动或尚无快照时）
    public static let snapshotRequestNotification = "com.anywhere.snapshot-request"
